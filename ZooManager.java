...
public class ZooManager {
    ...
    */
    public static void zooSchedule(int dayOfWeek, boolean isPremium,
        ...
        {
            System.out.print("Today's animals:\n" + AMPHIBIANS + "\n" + MAMMALS);
            if(isPremium)
            {
                System.out.println("\n" + POLAR_BEARS);
            }
    ...
    public static String feedAnimals(int dayOfWeek, int hourOfDay) {
        ...
        if(dayOfWeek<1||dayOfWeek>7||hourOfDay>23||hourOfDay<0)
        {
            feedingResult = INVALID_INPUT;
        }
        else if(dayOfWeek==7)
        {
            feedingResult = NO_FEED_SUNDAY;
        }
        else 
        {
            if(hourOfDay<=10&&hourOfDay>=8)
            {
                feedingResult = FEED_AMPH_MAMM;
            }
            else if(hourOfDay<=14&&hourOfDay>=12)
            {
                feedingResult = FEED_BIRD_REPT;
            }
            else 
            {
                feedingResult = NO_FEEDING;
            }
        }
